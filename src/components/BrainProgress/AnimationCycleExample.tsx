...
/**
 * AnimationCycleExample - A component that animates the brain SVG
 * with a continuous fill-pause-drain-pause cycle
 */
const AnimationCycleExample: React.FC<Omit<BrainProgressProps, 'totalPercent' | 'value' | 'maxValue'>> = ({
  ...
  // Create CSS for smooth transitions and proper filling
  useEffect(() => {
    ...
    style.textContent = `
      ...
      /* Ensure vivid gradients when filled */
      .brain-path[data-filled="true"] {
        fill-opacity: 1;
        stroke-opacity: 1;
      }
      
      .animation-cycle-example[data-paused="true"] .brain-path {
        transition: none; /* Disable transitions when paused */
      }
    `;
  ...
  // Set up the GSAP animation sequence
  useEffect(() => {
    ...
    const timeline = gsap.timeline({ 
      ...
      paused: isPaused,
      onUpdate: () => {
        // Calculate progress percentage based on timeline progress
        // Map the 0-1 timeline progress to 0-100 percent
        const calculatedProgress = Math.round(timeline.progress() * 100);
        setProgressPercent(calculatedProgress);
      }
    });
    
    // Set duration based on animation speed
    const duration = animationSpeed ? animationSpeed : 1; // Default 1s if not set
    const pauseDuration = 0.8; // Pause duration at full/empty state (in seconds)
    ...
    const animationOrder = [
      "path-4",   // Top arc (first to appear in storyboard)
    ...
    paths.forEach(path => {
      path.setAttribute('data-filled', 'false');
      gsap.set(path, { 
        fillOpacity: 0, 
        opacity: 0,
    ...
    // Filling animation (0% → 100%)
    const fillTimeline = gsap.timeline();
    
    // Animate each path in the specified order
    animationOrder.forEach((pathId, index) => {
      ...
      if (path) {
        fillTimeline.to(path, { 
          ...
          ease: "power1.inOut" 
        }, index * (duration * 0.2)); // Stagger with overlap
      }
    });
    
    // Full fill state - ensure all paths are 100% visible
    fillTimeline.to(paths, {
      fillOpacity: 1,
      opacity: 1,
      duration: 0.2,
      attr: { 'data-filled': 'true' },
      onComplete: () => {
        // Ensure 100% fill is visually accurate
        paths.forEach(path => {
    ...
    // Pause at full fill
    fillTimeline.to({}, { duration: pauseDuration });
    
    // Draining animation (100% → 0%)
    ...
    [...animationOrder].reverse().forEach((pathId, index) => {
      ...
      if (path) {
        drainTimeline.to(path, { 
          ...
          duration: duration * 0.8, 
          ease: "power1.inOut" 
        }, index * (duration * 0.15)); // Slightly faster unwinding
      }
    });
    
    // Pause at empty state
    drainTimeline.to({}, { duration: pauseDuration * 0.5 });
    
    // Add sequences to the main timeline
    timeline.add(fillTimeline).add(drainTimeline);
    
    // Store reference and start if not paused
  ...
  return (
    <div 
      className="brain-progress-container animation-cycle-example"
      style={{ 
        width, 
        height,
        position: 'relative',
    ...
    >
      ...
      <div className="animation-controls">
        <p>Fill ➡️ Pause ➡️ Unwind ➡️ Repeat</p>
      </div>
    </div>
  );
};
